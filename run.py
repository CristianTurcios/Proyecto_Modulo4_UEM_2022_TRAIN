...
def root():
    ...
    return {'Proyecto':'Mod. 4 - Ciclo de vida de modelos IA Training'}
...
def train_model_route():
    ...
    has_error = train_model.training_pipeline(df_path)

    if(has_error): 
        return {'TRAINING MODEL': 'Something went wrong'}

    # Se puede devolver lo que queramos (mensaje de éxito en el entrenamiento, métricas, etc.)
    return {'TRAINING MODEL': 'Modelo Entrenado Satisfactoriamente'}
